...
export default function Dashboard({ onNavigate, onLogout }: DashboardProps) {
  ...
  const [expandedReviewCardIds, setExpandedReviewCardIds] = useState<Record<number, boolean>>({});
  ...
  const toggleCardReviews = (gameId: number) => {
    setExpandedReviewCardIds((current) => ({
      ...current,
      ...
      <main className="pt-28 px-6 md:px-10 pb-20">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-10 md:mb-12">
            <h1 className="text-4xl md:text-6xl font-bold mb-3 bg-gradient-to-r from-white via-slate-100 to-slate-400 bg-clip-text text-transparent tracking-tight">
              Game Library
            </h1>
            <p className="text-white/65 text-base md:text-xl max-w-3xl">
              Choose your next adventure from our collection of thrilling games
            </p>
          </div>

          {/* Search and Filters */}
          <div className="space-y-5 mb-10 md:mb-12 rounded-3xl border border-white/10 bg-black p-5 md:p-6">
            {/* Search Bar */}
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-black border border-white/10 rounded-2xl pl-14 pr-6 py-4 text-white placeholder-white/40 focus:outline-none focus:border-cyan-500/50 focus:bg-black transition-all duration-300"
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
            {filteredGames.map((game, index) => (
              <div
                key={game.id}
                className="rounded-3xl overflow-hidden group border border-white/10 bg-black hover:-translate-y-1 hover:border-cyan-300/30 transition-all duration-300 hover:shadow-2xl hover:shadow-cyan-900/30"
                onClick={() => toggleCardReviews(game.id)}
                style={{
                  animation: `fadeInUp 0.6s ease-out ${index * 0.1}s both`
                }}
              >
                {/* Game Image */}
                <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-white/10 to-white/5">
                  <img
                    src={game.image}
                    alt={game.name}
                ...
                {/* Game Info */}
                <div className="p-6">
                  <h3 className="text-2xl font-bold mb-2 tracking-tight group-hover:text-cyan-300 transition-colors duration-300">
                    {game.name}
                  </h3>
                  <p className="text-sm text-white/60 mb-4 leading-relaxed">{game.description}</p>

                  {/* Game Meta */}
                  <div className="flex items-center gap-2.5 mb-5 text-xs text-white/50">
                  ...
                  <div className="flex gap-3">
                    <a
                      ...
                      onClick={(event) => {
                        ...
                        handleGameClick(game, event);
                      }}
                      className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-sm font-semibold rounded-xl px-4 py-3 flex items-center justify-center gap-2 hover:from-cyan-400 hover:to-blue-400 transition-all duration-300 shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40"
                    >
                      Play Now
                    ...
                    <a
                      ...
                      className="bg-white/10 text-white text-sm font-semibold rounded-xl px-4 py-3 flex items-center justify-center gap-2 hover:bg-white/20 transition-all duration-300 border border-white/20 hover:border-white/40"
                    >
                      <Code size={16} />
                    </a>
                  </div>

                  {expandedReviewCardIds[game.id] && (
                    <div className="mt-5 border-t border-white/10 pt-5" onClick={(event) => event.stopPropagation()}>
